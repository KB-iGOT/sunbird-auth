...
public class SunbirdModelUtils {
    ...
    public static UserModel getUserByNameEmailOrPhone(AuthenticationFlowContext context,
        ...
        if (username.matches(numberRegex)) {
            // Use searchForUserByUserAttributeStream and collect to List
            List<UserModel> userModels = session.users()
                    .searchForUserByUserAttributeStream(context.getRealm(),
                            KeycloakSmsAuthenticatorConstants.ATTR_MOBILE, username)
