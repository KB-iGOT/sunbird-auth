...
public class KeycloakSmsAuthenticator implements Authenticator {
    ...
    protected CODE_STATUS validateCode(AuthenticationFlowContext context) {
        ...
        List<CredentialModel> creds = new UserCredentialManager(session, context.getRealm(), context.getUser())
                .getStoredCredentialsByTypeStream(KeycloakSmsAuthenticatorConstants.USR_CRED_MDL_SMS_CODE)
                .collect(Collectors.toList());

        if (creds.isEmpty()) { // Changed from codeCreds to creds
            logger.debug("No SMS code found for user");
            return CODE_STATUS.INVALID;
        }

        CredentialModel expectedCode = creds.get(0); // Changed from codeCreds to creds
